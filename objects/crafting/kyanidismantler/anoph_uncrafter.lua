...
--If you'd like to make recipes for your mod which can't be deconstructed, make a patch for /objects/crafting/anoph_uncrafter/anoph_uncrafter.lua that adds '"itemname" : true' into the "recipeBlacklist" field, or add the "nouncrafting" tag to the recipe.
	
	matterBlacklist = config.getParameter("matterBlacklist")--these materials will not be turned into grey goo
...
function processOtherItems(inputItem)
	...
	elseif inputItem.name == "sapling" then
		...
		uncraftInput(outputList,1)
	elseif inputItem.name == "liquidmaterial" or inputItem.name == "matterblock" then
		return false

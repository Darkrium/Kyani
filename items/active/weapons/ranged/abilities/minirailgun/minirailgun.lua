...
MiniRailgun = WeaponAbility:new()

function MiniRailgun:init()
...
function MiniRailgun:update(dt, fireMode, shiftHeld)
...
function MiniRailgun:cooldown(duration)
...
function MiniRailgun:fireProjectile()
...
function MiniRailgun:firePosition()
  return vec2.add(mcontroller.position(), activeItem.handPosition(self.weapon.muzzleOffset))
end

function MiniRailgun:aimVector(angleAdjust, inaccuracy)
  local aimVector = vec2.rotate({1, 0}, self.weapon.aimAngle + angleAdjust + sb.nrand(inaccuracy, 0))
  aimVector[1] = aimVector[1] * mcontroller.facingDirection()
  return aimVector
end

function MiniRailgun:currentChargeLevel()
